...
# Define main window
root = tk.Tk()
root.title("Shop-Hisaab")
root.geometry("900x550")
root.configure(bg="#F8F9FA")  # Light theme background

# Sidebar Frame
sidebar = tk.Frame(root, bg="#2C3E50", width=220, height=550)
...
icons = {name: load_icon(path) for name, path in icon_paths.items()}

# Sidebar Buttons
buttons = ["Dashboard", "Sales", "Inventory", "Expenses"]
for btn in buttons:
    frame = tk.Frame(sidebar, bg="#34495E", height=50)
    ...
    if btn == "Inventory":
        button.config(command=lambda: show_inventory())
    if btn == "Sales":
        button.config(command=lambda: show_sales())
